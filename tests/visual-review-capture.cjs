...
const OUTPUT_DIR = path.join(ROOT_DIR, "visual-review");
...
async function prepareContext(browser, { mobile = false, signedIn = false } = {}) {
...
async function waitForSettled(page, delay = 900) {
  try {
...
async function setupIndexReview(page) {
  await page.evaluate(() => {
    const fields = {
      phone: "(305) 555-0188",
      ...
      businessContact: "owner@example.com | (305) 555-0188",
      notes: "Please call when you are on the way."
    };
    ...
    const reviewIndex = Array.from(document.querySelectorAll(".wizard-step")).findIndex(step => step.dataset.field === "consent");
    if (typeof window.setStep === "function" && reviewIndex >= 0) {
      window.setStep(reviewIndex, "forward");
    }

    if (typeof window.refreshFormState === "function") {
      window.refreshFormState();
    }
  });
}

async function capturePage(context, baseUrl, shot) {
  const page = await context.newPage();
  await page.goto(`${baseUrl}/${shot.path}`, { waitUntil: "domcontentloaded" });
  await waitForSettled(page, shot.delay || 900);

  if (typeof shot.setup === "function") {
    await shot.setup(page);
    await waitForSettled(page, shot.postSetupDelay || 1200);
  }

  const targetPath = path.join(OUTPUT_DIR, shot.fileName);
...
async function main() {
  ...
  try {
    const desktopPublic = await prepareContext(browser, { mobile: false, signedIn: false });
    const mobilePublic = await prepareContext(browser, { mobile: true, signedIn: false });
    const desktopSigned = await prepareContext(browser, { mobile: false, signedIn: true });
    const mobileSigned = await prepareContext(browser, { mobile: true, signedIn: true });
    const baseUrl = `http://127.0.0.1:${port}`;

    const shots = [
      { context: desktopPublic, path: "about.html", fileName: "desktop-about.png" },
      { context: desktopPublic, path: "privacy.html", fileName: "desktop-privacy.png" },
      { context: desktopPublic, path: "terms.html", fileName: "desktop-terms.png" },
      { context: desktopPublic, path: "signin.html", fileName: "desktop-signin.png" },
      { context: mobilePublic, path: "about.html", fileName: "mobile-about.png" },
      { context: mobilePublic, path: "privacy.html", fileName: "mobile-privacy.png" },
      { context: mobilePublic, path: "terms.html", fileName: "mobile-terms.png" },
      { context: mobilePublic, path: "signin.html", fileName: "mobile-signin.png" },
      { context: desktopPublic, path: "index.html", fileName: "desktop-index-public.png" },
      { context: mobilePublic, path: "index.html", fileName: "mobile-index-public.png" },
      { context: desktopSigned, path: "index.html", fileName: "desktop-index-review.png", setup: setupIndexReview, postSetupDelay: 1600 },
      { context: mobileSigned, path: "index.html", fileName: "mobile-index-review.png", setup: setupIndexReview, postSetupDelay: 1600 },
      { context: desktopSigned, path: "account.html", fileName: "desktop-account.png" },
      { context: desktopSigned, path: "account-info.html", fileName: "desktop-account-info.png" },
      { context: desktopSigned, path: "settings.html", fileName: "desktop-settings.png" },
      { context: desktopSigned, path: "client-details.html", fileName: "desktop-client-details.png" },
      { context: desktopSigned, path: "calendar.html", fileName: "desktop-calendar.png", delay: 1200 },
      { context: desktopSigned, path: "branding.html", fileName: "desktop-branding.png", delay: 1200 },
      { context: desktopSigned, path: "form-creator.html", fileName: "desktop-form-creator.png", delay: 1200 },
      { context: mobileSigned, path: "account.html", fileName: "mobile-account.png" },
      { context: mobileSigned, path: "account-info.html", fileName: "mobile-account-info.png" },
      { context: mobileSigned, path: "settings.html", fileName: "mobile-settings.png" },
      { context: mobileSigned, path: "client-details.html", fileName: "mobile-client-details.png" },
      { context: mobileSigned, path: "calendar.html", fileName: "mobile-calendar.png", delay: 1200 },
      { context: mobileSigned, path: "branding.html", fileName: "mobile-branding.png", delay: 1200 },
      { context: mobileSigned, path: "form-creator.html", fileName: "mobile-form-creator.png", delay: 1200 }
    ];

    for (const shot of shots) {
      await capturePage(shot.context, baseUrl, shot);
    }

    await desktopPublic.close();
    await mobilePublic.close();
    await desktopSigned.close();
    await mobileSigned.close();
  } finally {
    await browser.close();
