...
test.describe("Navigation chrome", () => {
  test("shows a dark loading intro and clears it after the form is ready", async ({ page }) => {
    ...
    await expect(page.locator(".container")).toHaveCSS("transform", "none");
    await expect(page.locator(".wizard-step.active .question-wrap")).toHaveCSS("transform", "none");
    await navigation;
  ...
  test("shows the dev badge with the deployed app version", async ({ page }) => {
    ...
    await expect(badge).toContainText("v20260501.10");
